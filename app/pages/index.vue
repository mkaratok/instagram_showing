    ...
    <main class="max-w-4xl mx-auto pb-24">
      ...
      <div v-else-if="activeTab === 'PROFİL'" class="px-4 py-8 sm:px-8 space-y-12 animate-fade-in">
        ...
        <section class="pt-8 border-t" :class="isDark ? 'border-earth-800' : 'border-gray-200'">
          ...
          <!-- Social Links Grid -->
          <div class="mb-8">
            <h3 class="text-sm font-medium mb-4" :class="isDark ? 'text-earth-400' : 'text-gray-500'">Bizi Takip Edin</h3>
            <SocialLinksGrid :isDark="isDark" />
          </div>
          
          <div class="grid md:grid-cols-2 gap-8 items-start">
            <div class="space-y-4 text-sm font-medium">
              <a v-if="settings.social?.instagram" :href="settings.social.instagram" target="_blank" class="flex items-center gap-3 hover:opacity-70 transition-opacity">
              ...
              <a v-if="settings.contact?.address" :href="`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(settings.contact.address)}`" target="_blank" class="flex items-center gap-3 hover:opacity-70 transition-opacity">
                ...
                <span>{{ settings.contact.city || 'Ordu' }}</span>
              </a>
            </div>
            <div v-if="settings.contact?.googleMapsEmbed" class="h-64 rounded-lg overflow-hidden" :class="isDark ? 'bg-earth-800' : 'bg-gray-200'">
              <iframe 
